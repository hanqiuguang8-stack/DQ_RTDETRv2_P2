...
A2_RESUME="output/rtdetrv2_r50vd_6x_visdrone_p2_dq_a2_cgfe/last.pth"

echo "[$(date +'%F %T')] resume DQ RTDETRv2 P2 VisDrone A2-A4"

mkdir -p "output/rtdetrv2_p2_dq_a2_cgfe"
echo "[$(date +'%F %T')] RESUME A2_CGFE cfg=configs/rtdetrv2/rtdetrv2_r50vd_6x_visdrone_p2_dq_a2_cgfe.yml resume=${A2_RESUME}" | tee -a "output/rtdetrv2_p2_dq_a2_cgfe/screen_train.log"
conda run --no-capture-output -n RTDETR torchrun --master_port=${MASTER_PORT} --nproc_per_node=2 tools/train.py -c "configs/rtdetrv2/rtdetrv2_r50vd_6x_visdrone_p2_dq_a2_cgfe.yml" -r "${A2_RESUME}" --use-amp --seed=0 >> "output/rtdetrv2_p2_dq_a2_cgfe/screen_train.log" 2>&1
